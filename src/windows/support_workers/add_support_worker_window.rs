                ...
                ui.horizontal(|ui| {
                    if ui.button("➕ ADD").clicked() {
                        let new_support_worker = SupportWorker {
                            ...
                            confidentiality_agreement: (!self.confidentiality_agreement.0).then(|| self.confidentiality_agreement.1.clone()),
                            police_clearance: (!self.police_clearance.0).then(|| self.police_clearance.1.clone()),
                            car_insurance: (!self.car_insurance.0).then(|| self.car_insurance.1.clone()),
